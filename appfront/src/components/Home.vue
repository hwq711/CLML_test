<template>
  <div class="home">
    <el-row v-loading="loading">
      <el-row>
        <el-input v-model="input" placeholder="请输入药物ID" style="display:inline-table; width: 30%;"/>
        <el-button type="primary" @click="search()" style="margin: 2px;">搜索</el-button>
      </el-row>
      <el-row style="margin-top: 2em">
        <el-radio v-model="radio" label="1">搜索药物</el-radio>
...
export default {
  ...
  methods: {
    ...
    search_drug () {
      this.$axios.get('http://127.0.0.1:8000/api/search_drug?drug_name=' + this.input)
        .then(res => {
          console.log(res)
          if (res.status === 200) {
            this.loading = false
