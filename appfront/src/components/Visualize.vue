  ...
  methods: {
    ...
    draw_chart () {
      ...
      my_chart.on('click', function (param) {
        ...
        if (param.dataType === 'node') {
          if (param.data.category.indexOf('DRUG') !== -1 && that.search_kind === '1') {
            that.loading = true
            console.log('DRUG:' + param.name)
            that.$axios.get('http://127.0.0.1:8000/api/search_drug?drug_name=' + param.name)
              .then(res => {
                console.log(res)
                if (res.status === 200) {
                  that.loading = false
    ...
    update_data () {
      ...
      if (this.search_kind === '1') {
        this.categories = [{name: 'DRUG'}, {name: 'PREDICT DRUG'}, {name: 'KNOWN DRUG'}]
        this.nodes.push({name: this.search_target, symbolSize: 100, category: 'DRUG'})
      }

      // eslint-disable-next-line camelcase
      let target_name = this.search_kind === '1' ? 'drug_name' : 'drug_name'

      for (let i = 0, len = this.predict_results.length; i < len; i++) {
        let item = this.predict_results[i]
        if (this.search_kind === '1') {
          this.nodes.push({name: item[target_name], symbolSize: 50, category: 'PREDICT DRUG'})
        }
        this.links.push(
          {
      ...
      for (let i = 0, len = this.known_results.length; i < len; i++) {
        ...
        if (this.search_kind === '1') {
          this.nodes.push({name: item[target_name], symbolSize: 50, category: 'KNOWN DRUG'})
        }
        this.links.push(
          {
            source: String(this.search_target),
